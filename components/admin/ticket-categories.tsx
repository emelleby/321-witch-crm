    ...
    try {
      const { data, error } = editingCategory
        ? await supabase
            ...
            .select()
        : await supabase.from('ticket_categories').insert([categoryForm]).select();
      ...
      notifications.error('Failed to perform operation');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleTagSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error } = editingTag
        ? await supabase.from('ticket_tags').update(tagForm).eq('id', editingTag).select()
                ...
                <form onSubmit={handleCategorySubmit} className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setCategoryForm({ ...categoryForm, parent_id: e.target.value || null })
                      }
