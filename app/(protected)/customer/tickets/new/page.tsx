...
export default function NewTicketPage() {
    ...
    const [similarContent, setSimilarContent] = useState<Array<{ title: string, content: string, type: 'article' | 'forum', url: string }>>([])
    ...
    const searchSimilarContent = async () => {
        ...
        try {
            ...
            console.error('Error searching similar content:', error instanceof Error ? error.message : 'Unknown error')
            setSimilarContent([])
        }
    }
                                ...
                                {similarContent.map((item, i) => (
                                    <div key={i} className="space-y-1">
                                        ...
                                        <span className="text-xs text-muted-foreground">
                                            From {item.type === 'article' ? 'Knowledge Base' : 'Forum'}
                                        </span>
                                    </div>
                                ))}
                            </div>
